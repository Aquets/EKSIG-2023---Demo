...
void main() {
    ...
    vec2 center = u_resolution * 2.0; // draw the shape at the center of the screen
    float size = u_resolution.y * 0.5; // make the shape a quarter of the screen height
    float sides = mod(floor(u_mouse.x), 7.0) + 3.0; // slowly increase the sides, when it reaches 10 sides, go back down to 3
    float rotation = u_time; // rotation is in radians, but for time it doesnt really matter

    vec2 st = gl_FragCoord.xy/u_resolution;

    float x = center.x * 2.0;
    float y = center.y * 2.0;

    vec2 coord = vTexCoord;

    vec3 color = vec3((coord.x + coord.y), y, x);
